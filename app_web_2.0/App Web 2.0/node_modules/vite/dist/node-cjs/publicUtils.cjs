...
function sanitizeContent(content, type) {
  ...
  // Solo sanitiza si el tipo es html o js (si es texto ejecutable)
  if (['html', 'js'].includes(type)) {
    return content
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
...
function send(req, res, content, type, options) {
  ...
  if (map && "version" in map && map.mappings) {
    ...
    if (convertSourceMap.mapFileCommentRegex.test(code)) {
      ...
      const safeUrl = cleanUrl(req.url);
	const urlWithoutTimestamp = removeTimestampQuery(safeUrl);
  ...
  const safeContent = sanitizeContent(content.toString(), type);
